<!-- 影片详情页 复用性少并属于影片-->
<template>
    <div id="detailContrainer" class="slide-enter-active"><!--自定义滑动详情slide-enter-active-->
		<!--Header组件-->
		<Header title="影片详情">
			<i class="iconfont icon-right" @touchstart="handleToBack"></i>
		</Header>
		<Loading v-if="isLoading"/><!--loading加载-->
...
export default {
	...
	data(){
		return {
			//根据json数据结构，返回的名称与类型，如下：名称detailMovie :类型对象{}
			//初始值
			detailMovie : {},
			isLoading : true
		}
	...
	mounted(){
		...
		this.axios.get('/api/detailmovie?movieId='+this.movieId).then((res)=>{
			...
			if(msg === 'ok'){
				this.isLoading = false;//加载数据后关闭

				//这里获取的是所有数据，可以选择性挂载到页面
